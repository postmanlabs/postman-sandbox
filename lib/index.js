    ...
    createContext (options, callback) {
        ...
        bootcode((err, code) => {
            ...
            const template = _.get(options, 'template'),
                chaiPlugin = _.get(options, 'chaiPlugin'),
                disableLegacyAPIs = _.get(options, 'disableLegacyAPIs', false),
                initOptions = template || chaiPlugin || disableLegacyAPIs ?
                    { template, chaiPlugin, disableLegacyAPIs } : {};

            new PostmanSandbox().initialize(initOptions, options, callback);
        });
    },
