var _ = require('lodash'),
    ...
    DEPENDENCIES_PATH = DEPENDENCIES_DIR + '/dependencies.txt',
...
Bundle = function (options) {
    ...
    dependencies = [];
    this.bundler.on('package', function (pck) {
        dependencies.push(pck.name + '@' + pck.version);
    });
...
_.assign(Bundle.prototype, /** @lends Bundle.prototype */ {
    compile: function (done) {
        return this.bundler.bundle(function (err, bundle) {
            if (bundlerHasRun) { return done(err, bundle); }
            bundlerHasRun = true;

            try {
                !fs.existsSync(DEPENDENCIES_DIR) && fs.mkdirSync(DEPENDENCIES_DIR);
                fs.writeFileSync(DEPENDENCIES_PATH, _.uniq(dependencies).sort().join('\n'));
            }
            catch (e) {
                console && console.warn(colors.red.bold('sandbox: ' + e.message + '\n' +
                    'Error while writing dependencies to the file'));
            }

            return done(err, bundle);
        });
    }
});
