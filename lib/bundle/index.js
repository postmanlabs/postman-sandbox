...
class Bundle {
    ...
    compile (done) {
        this.bundler.bundle((err, bundle) => {
            ...
            // Scrub process object to prevent access to native backdoors
            const scrubProcess = `
                    if (typeof process !== 'undefined') {
                        const propsToDel = ['binding', '_linkedBinding', 'dlopen', 'getBuiltinModule', 'report'];
                        for (const key of propsToDel) {
                            delete process[key];
                        }
                    }
                `,
                ...
                bundleString = scrubProcess + safeExposeBuiltIns + bundle.toString();
