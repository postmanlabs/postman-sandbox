...
class Execution {
    constructor (id, event, context, options) {
        ...
        this.getResponseClass = options.getResponseClass;
        this.event = event;
        ...
        if (options.initializeExecution) {
            initializationResult = options.initializeExecution(event.listen, context);

            const { request, response, message } = initializationResult || {};
