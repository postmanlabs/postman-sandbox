...
module.exports = function (scope, code, execution, console, timers, pmapi, tests) {
    ...
    if (NONLEGACY_SANDBOX_MARKERS[code.substr(0, 15)]) {
        ...
        // inject tests object only in legacy mode. This object will be mutated
        scope.import({tests: tests});
    }
    ...
    scope.exec(code, function (err) {
        ...
        // Raise assertion events for the sync tests
        !_.isEmpty(tests) && legacy.raiseAssertionEvents(execution, pmapi, tests);
