var _ = require('lodash'),
    ...
    WHITE_LISTED_PROPERTIES = ['id', 'target', 'legacy', 'cursor', 'data', 'cookies', 'environment', '_variables',
        'globals', 'collectionVariables', 'return'],
...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        timers = new PostmanTimers(null, function (err) {
            ...
            (dnd !== true) && bridge.dispatch(executionEventName, err || null,
                // remove any extra properties which are not supposed to be sent back
                _.pick(execution.toJSON(), WHITE_LISTED_PROPERTIES));
        });
        ...
        bridge.on(responseEventName, function (id, err, res) {
