...
module.exports = function (bridge, glob) {
    ...
    bridge.once('initialize', ({ template, ...initOptions }) => {
        ...
        scope.exec(template, (err) => {
            ...
            const { chaiPlugin, initializeExecution: setupExecution, getResponseClass: responseClassGetterFn } =
                (_module && _module.exports) || {};

            if (_.isFunction(chaiPlugin)) {
                // TODO: Figure out branching logic for chai plugin based on the target being used.
                chai.use(chaiPlugin);
            }
            ...
            if (_.isFunction(responseClassGetterFn)) {
                getResponseClass = responseClassGetterFn;
            }
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        const executionEventName = EXECUTION_RESULT_EVENT_BASE + id,
            ...
            execution = new Execution(id, event, context, { ...options, initializeExecution, getResponseClass }),
        ...
        bridge.on(runRequestResponseEventName, function (id, err, response, results) {
            ...
            const { variableMutations = {} } = results || {};
