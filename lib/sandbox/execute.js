const _ = require('lodash'),
    ...
    EXECUTION_SKIP_EVENT = 'execution.skip.',
...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        const executionEventName = EXECUTION_RESULT_EVENT_BASE + id,
            ...
            executionSkipEvent = EXECUTION_SKIP_EVENT + id,
        ...
        executeContext(scope, code, execution,
            ...
            (
                ...
                /* onSkipRequest = */ () => {
                    // throw an error if skipRequest is called from test script
                    // as we only want to provide this api in prerequest script
                    if (execution && execution.target === 'test') {
                        throw new ReferenceError('skipRequest is not defined');
                    }

                    bridge.dispatch(executionSkipEvent, options.cursor);
