...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        bridge.on(runRequestResponseEventName, function (id, err, response, results) {
            ...
            const { variableMutationStages = {} } = results || {};

            ['prerequest', 'test'].forEach(function (scriptStage) {
                const variableMutationForThisStage = variableMutationStages[scriptStage];

                if (!variableMutationForThisStage) {
                    return;
                }

                ['_variables', 'environment', 'globals'].forEach(function (variableType) {
                    let mutableVariableScope = execution[variableType],
                        mutationsMade = variableMutationForThisStage[variableType];
            ...
            timers.clearEvent(id, err, response);
        });
