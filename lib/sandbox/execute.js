const _ = require('lodash'),
    ...
    EXECUTION_SYNC_VARIABLES_FROM_ANOTHER_EXECUTION = 'execution.sync_variables_from_another_execution.',
...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        const executionEventName = EXECUTION_RESULT_EVENT_BASE + id,
            ...
            variableSyncingEventName = EXECUTION_SYNC_VARIABLES_FROM_ANOTHER_EXECUTION + id,
            requestForVariablesEventName = EXECUTION_GET_VARIABLES_FROM_THIS_EXECUTION + id,
        ...
        timers = new PostmanTimers(null, function (err) {
            ...
            function complete () {
                ...
                bridge.off(variableSyncingEventName);
                bridge.off(requestForVariablesEventName);
        ...
        bridge.on(variableSyncingEventName, function (resultVariablesFromOtherExecution) {
            ['_variables', 'environment', 'globals'].forEach(function (variableType) {
                let mutableVariableScope = execution[variableType],
                    mutationsMade = resultVariablesFromOtherExecution[variableType];
                ...
                if (_.isFunction(mutableVariableScope.applyMutation)) {
                    mutationsMade = new sdk.MutationTracker(mutationsMade);

                    mutationsMade.applyOn(mutableVariableScope);
                }
            });
        });

        // For a parallel execution to receive variable values from this execution
        bridge.on(requestForVariablesEventName, function (requestUniqueId) {
            let valuesToSendBack = {};

            ['_variables', 'environment', 'globals', 'collectionVariables'].forEach(function (variableType) {
                valuesToSendBack[variableType] = execution[variableType].values;
            });

            bridge.dispatch(EXECUTION_REPLY_TO_VARIABLES_REQUEST + requestUniqueId, valuesToSendBack);
        });
                ...
                new PostmanAPI(execution,
                    ...
                    function onRunCollectionRequest (requestToRunId, requestOptions, callback) {
                        ...
                        bridge.dispatch(executionRunCollectionRequestEventName,
                            options.cursor, id, eventId, requestToRunId, requestOptions);
                    },
