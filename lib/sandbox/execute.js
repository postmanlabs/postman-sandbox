...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        var cursor = options.cursor,
            ...
            dispatched = false, // @todo get rid of flag based logic
            timers;
        ...
        if (_.isFinite(options.timeout)) {
            setTimeout(function () {
                if (!dispatched) {
                    timers.seal();
                    dispatched = true;
                    bridge.dispatch(executionEventName, new Error('sandbox: execution timeout'), execution);
                }
            }, options.timeout);
        }

        // create the controlled timers
        timers = new PostmanTimers(null, null, function () {
            if (!dispatched) {
                dispatched = true;
                bridge.dispatch(executionEventName, null, execution);
            }
        }, function (err) {
