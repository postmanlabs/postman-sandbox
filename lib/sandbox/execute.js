...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        if (!(code && _.isString(code))) {
            return bridge.dispatch(executionEventName, null, legacy.transformExecution(execution)); // @todo remove leg
        }
        ...
        scope.exec(code, function (err) {
            ...
            bridge.dispatch(executionEventName, err, legacy.transformExecution(execution)); // @todo remove legacy
        });
    });
};
