const _ = require('lodash'),
    ...
    executionCallbacksStore = require('./utils/callbacks-store'),
    ...
    EXECUTION_RUN_REQ_RESPONSE_EVENT_BASE = 'execution.run_collection_request_response.',
...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        const executionEventName = EXECUTION_RESULT_EVENT_BASE + id,
            ...
            runCollectionRequestResponseEventName = EXECUTION_RUN_REQ_RESPONSE_EVENT_BASE + id,
        ...
        timers = new PostmanTimers(null, function (err) {
            ...
            function complete () {
                // When sharing sandbox contexts between one or more request
                // (When a request is nested in another requests's scripts,
                //  we don't want to turn off event listeners till the main request finishes executing)
                if (!options.isNestedSandboxExecution) {
                    vault.dispose();
                    bridge.off('uncaughtException', onError);
                }
                bridge.off(abortEventName);
                bridge.off(responseEventName);
                bridge.off(runCollectionRequestResponseEventName);
        ...
        bridge.on(responseEventName, function (eventId, err, res, history) {
            const registeredCallbackForThisEvent = executionCallbacksStore.getCallback(eventId);

            if (typeof registeredCallbackForThisEvent === FUNCTION) {
                registeredCallbackForThisEvent(err, res, history);
            }
        ...
        bridge.on(cookiesEventName, function (eventId, err, res) {
            const registeredCallbackForThisEvent = executionCallbacksStore.getCallback(eventId);

            if (typeof registeredCallbackForThisEvent === FUNCTION) {
                registeredCallbackForThisEvent(err, res);
            }
            ...
            (
                new PostmanAPI(execution,
                    function onRequest (request, callback) {
                        const eventId = executionCallbacksStore.registerCallback(callback);
