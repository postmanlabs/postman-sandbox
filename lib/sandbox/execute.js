...
module.exports = function (bridge, glob) {
     ...
     * @param {String} [options.template] - Template string or map of template strings to protocol names
     * @param {String} [options.chaiPlugin]
     * @param {Boolean} [options.disableLegacyAPIs]
     * @param {Array.<String>} [options.disabledAPIs]
     */
    bridge.once('initialize', async ({ template, chaiPlugin, ...initOptions }) => {
        initializationOptions = initOptions || {};

        let templates = null,
            chaiPluginInitialized = false;

        if (template && typeof template === 'string') {
            templates = { _default: template };
        }
        else if (template && typeof template === 'object' && Object.keys(template).length > 0) {
            templates = template;
        }

        // If no custom template is provided, go ahead with the default steps
        if (!templates && !chaiPlugin) {
        ...
        for (const templateName of Object.keys(templates || {})) {
            const template = templates[templateName],
                ...
                templateExecResult = await new Promise((resolve) => {
                    scope.exec(template, (err) => {
                        if (err) {
                            return resolve({ error: err });
                        }

                        const templateExports = (_module && _module.exports) || {};

                        templatesRegistry[templateName] = templateExports;

                        if (_.isFunction(templateExports.chaiPlugin) && !chaiPluginInitialized) {
                            chaiPluginInitialized = true;
                            chai.use(templateExports.chaiPlugin);
                        }
            ...
            if (templateExecResult.error) {
                return bridge.dispatch('initialize', templateExecResult.error);
            }
        }

        if (chaiPlugin && typeof chaiPlugin === 'string' && !chaiPluginInitialized) {
            const chaiPluginExecResult = await new Promise((resolve) => {
                scope.exec(chaiPlugin, (err) => {
                    if (err) {
                        return resolve({ error: err });
                    }
            ...
            if (chaiPluginExecResult.error) {
                return bridge.dispatch('initialize', chaiPluginExecResult.error);
            }
        }
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        const executionEventName = EXECUTION_RESULT_EVENT_BASE + id,
            ...
            execution = new Execution(id, event, context, { ...options, getProtocolMetadata }),
