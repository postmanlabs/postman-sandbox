...
module.exports = function (bridge, glob) {
    ...
    bridge.once('initialize', ({ template, ...initOptions }) => {
        ...
        scope.exec(template, (err) => {
            ...
            const {
                ...
                protocolToTemplatesMap: protocolToTemplates,
                getProtocolTemplate: protocolTemplateGetter
            ...
            if (_.isEmpty(protocolToTemplates)) {
                protocolToTemplatesMap = protocolToTemplates;
            }

            if (_.isFunction(protocolTemplateGetter)) {
                getProtocolTemplate = protocolTemplateGetter;
            }

            if (_.isFunction(chaiPlugin)) {
                chai.use((chai, utils) => {
                    return chaiPlugin(chai, utils, protocolToTemplatesMap || {});
                });
            }
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        const executionEventName = EXECUTION_RESULT_EVENT_BASE + id,
            ...
            execution = new Execution(id, event, context, { ...options, initializeExecution, getProtocolTemplate }),
