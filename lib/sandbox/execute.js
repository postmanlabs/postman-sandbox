...
module.exports = function (bridge, glob) {
    ...
    bridge.on('execute', function (id, event, context, options) {
        ...
        // handle run collection request response event from outside sandbox
        bridge.on(runCollectionRequestResponseEventName, function (id, err, res, history) {
            timers.clearEvent(id, err, res, history);
        });
        ...
        executeContext(scope, code, execution,
            ...
            (
                new PostmanAPI(execution,
                    ...
                    function onRunCollectionRequest (requestId, requestOptions, callback) {
                        var eventId = timers.setEvent(callback);

                        bridge.dispatch(executionRunCollectionRequestEventName,
                            options.cursor, id, eventId, requestId, requestOptions);
                    },
