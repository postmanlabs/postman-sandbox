...
module.exports = function (bridge, glob) {
     ...
     */
    bridge.once('initialize', async ({ template, templates, chaiPlugin, ...initOptions }) => {
        ...
        const templateExecPromises = [],
            availableTemplates = templates || (template ? { _default: template } : {}),
            templateNames = Object.keys(availableTemplates);

        for (const name of templateNames) {
            templateExecPromises.push(execTemplate(availableTemplates[name]));
        }

        try {
            let templateExecResults = await Promise.all(templateExecPromises),
                chaiPlugins = [];

            for (let i = 0; i < templateExecResults.length; i++) {
                const templateExecResult = templateExecResults[i],
                    templateName = templateNames[i];

                templatesRegistry[templateName] = templateExecResult;

                if (_.isFunction(templateExecResult.chaiPlugin) && !chaiPlugin) {
                    chaiPlugins.push(templateExecResult.chaiPlugin);
                }
            }

            if (chaiPlugins.length > 1) {
                return bridge.dispatch('initialize', new Error('sandbox: multiple chai plugins are not supported'));
            }
            else if (chaiPlugins.length === 1) {
                chai.use(chaiPlugins[0]);
            }
        }
        catch (error) {
        ...
        if (chaiPlugin && typeof chaiPlugin === 'string') {
            try {
                const exposedChaiPlugin = await execTemplate(chaiPlugin);
