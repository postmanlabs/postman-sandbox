...
module.exports = function (chai) {
    ...
    // @note directly tv4 assertions since chai-json-schema does not play well inside VM.
    Assertion.addMethod('bodyWithJSONSchema', function (schema) {
        new Assertion(this._obj).to.be.postmanResponse;
        new Assertion(schema).to.be.an('object');
        new Assertion(this._obj).to.have.jsonBody();

        var result = tv4.freshApi().validateResult(this._obj.json(), schema);

        this.assert(result.valid,
            'expect response body json to match schema but got #{act}',
            'expected response json to not match schema',
            null, result);
    });
