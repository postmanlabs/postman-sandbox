 ...
 */
function Timerz (executionId, delegations, onError, onAnyTimerStart, onAllTimerEnd) {
    var /**
    ...
    (typeof timers.setEvent !== FUNCTION) && (function () {
        ...
        timers.setEvent = function (callback, executionId) {
            var id = `${executionId}.${++total}`;
        ...
        timers.clearAllEvents = function (executionId) {
            Object.keys(events).forEach(function (prop) {
                if (prop.includes(executionId)) {
                    delete events[prop];
                }
            });
        };
    ...
    timerFunctionNames.forEach(function (name) {
        ...
        this[('set' + name)] = function (callback) {
            ...
            var id = `${executionId}.${++total}`, // get hold of the next timer id
            ...
            running[id] = timers[('set' + name)].apply(this, name === 'Event' ? [args[0], executionId] : args);
    ...
    this.clearAll = function () {
        ...
        timerFunctionNames.forEach(function (name) {
            this[('clearAll' + name + 's')](executionId);
        }.bind(this));
    };
