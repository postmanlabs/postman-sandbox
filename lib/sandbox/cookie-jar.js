...
PostmanCookieJar.prototype.get = function (url, name, callback) {
    ...
    this.jar.getCookies(url, function (err, cookies) {
        ...
        for (i = 0, ii = cookies.length; i < ii; i++) {
            ...
            // @todo for cookies with the same path length, the cookie with the
            // earliest creation time will be returned.
            if (cookies[i].key === name && String(cookies[i].path || '').length >= path.length) {
                path = String(cookies[i].path || '');
                cookie = deserialize(cookies[i]);
            }
        }

        callbackHandler(callback, null, cookie && cookie.valueOf());
    });
};
