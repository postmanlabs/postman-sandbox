...
function Postman (execution, onRequest, onSkipRequest, onAssertion, cookieStore, options = {}) {
    ...
    _assignDefinedReadonly(this, /** @lends Postman.prototype */ {
        ...
        * Contains information pertaining to the script execution
        ...
        /**
         * Require a package from npm in the script asynchronously.
         *
         * @param {String} package - Package name. Must be in the format `npm:<package-name>@<version>`.
         * If version is not provided, it defaults to `latest`.
         * @returns {Promise<*>} Promise that resolves to the required package
         *
         * @example
         * const _ = await pm.require('npm:lodash');
         * console.log(_.isEmpty([1, 2, 3]));
         *
         */
        require: function (package) {
            if (!package) { throw new Error('Invalid package name'); }

            const [pkgScheme, pkg] = package.split(':');

            if (pkgScheme !== 'npm' || !pkg) { throw new Error('Invalid package scheme'); }

            // eslint-disable-next-line one-var
            const [pkgName, pkgVersion = 'latest'] = pkg.split('@'),
                BROWSERIFY_CDN = `${options.cdn}/module/${pkgName}/version/${pkgVersion}`;

            if (!pkgName) { throw new Error('Invalid package name'); }

            return new Promise((resolve, reject) => {
                onRequest(new PostmanRequest(BROWSERIFY_CDN), (err, response) => {
                    if (err) {
                        return reject(err);
                    }

                    const package = (
                        PostmanResponse.isResponse(response) ? response : (new PostmanResponse(response))
                    ).text();

                    // eslint-disable-next-line security/detect-eval-with-expression, no-eval
                    eval(package);

                    resolve(require(pkgName));
                });
            });
        },
