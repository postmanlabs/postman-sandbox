        ...
        */
        ...
        execution: /** @lends Execution */ {
            ...
            setNextRequest: function setNextRequest (request) {
                // Nested requests do not get the privilege of invoking setNextRequest to alter execution order.

                // Adding an explicit check as our setup doesn't allow nested API properties to be disabled
                // Furthermore, disabling an API causes calls to it to error out. Do we want that or just a noOp?

                // TODO: Should we have this as an explicit check or a generalized wrapper around all nested functions?
                // TODO: Decide if we want to use the disabledAPIs array or a new options.noOps field.
                if (options.disabledAPIs.includes('execution.setNextRequest')) {
                    return;
                }
             ...
             *
             * @param {String} requestId - id of the collection request to run
             * @param {{ timeout?: number, variables?: Record<string, string | number> }} options - Options to enforce
             * @param {Function} [callback] - callback function
             * @returns {Promise<Response>|undefined} - returns a promise if callback is not provided
             */
            runRequest: function (requestId, options, callback) {
                let history;
